...
int Writer(void* data) {
    ...
    for (i = 0; i < WRITER_TURNS; i++) {
	    ...
	    // Write
	    	...
	    	printf("(W) Writer have just written -------------------[BUFOR] = %d\n", buf);
       		wroteCount++;
  ...
  //  sem_init(&sem_reader, 0, 5);
    ...
    for (i = 0; i < READER_TURNS; i++) {
      ...
      /*  sem_wait(&sem_reader);                      //wait
        ...
        if (readersCount == 1){
            sem_wait(&sem_writer);                  //writer is waiting        if there is any  
            printf("(W) Writers are waiting\n");
        }
...
*/
	    ...
	    // Read
	    	...
	    	printf("(R) Reader have just read -------------------[BUFOR] = %d\n", buf);
